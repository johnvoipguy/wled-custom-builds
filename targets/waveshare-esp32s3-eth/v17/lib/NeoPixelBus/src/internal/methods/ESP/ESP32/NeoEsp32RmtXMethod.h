...
public:
    ...
    void Initialize()
    {
        ...
        // DIAG/FIX: was 192 - on ESP32-S3 the RMT TX peripheral has only 4 channels
        // sharing a total of ~192 symbol-words of on-chip memory (48 per channel).
        // Requesting 192 symbols for the FIRST channel alone consumes the entire
        // shared pool, silently starving rmt_new_tx_channel() for every channel
        // after it (return value was never checked - see ret logging below).
        // 48 = SOC_RMT_MEM_WORDS_PER_CHANNEL on S2/S3, one full block per channel,
        // so 4 channels fit exactly without contention. The encoder refills this
        // block incrementally via interrupt (RMT_ENCODING_MEM_FULL), so a small
        // block does not limit frame size.
        ...
        esp_err_t retChan = rmt_new_tx_channel(&config, &_channel);
        led_strip_encoder_config_t encoder_config = {};
        encoder_config.resolution = T_SPEED::RmtTicksPerSecond;

        _tx_config.loop_count = 0; //no loop

        esp_err_t retEnc = rmt_new_led_strip_encoder(&encoder_config, &_led_encoder, T_SPEED::RmtBit0, T_SPEED::RmtBit1);

        esp_err_t retEnable = rmt_enable(_channel);
        if (retChan != ESP_OK || retEnc != ESP_OK || retEnable != ESP_OK) {
            Serial.printf("DBG RMT Initialize FAILED pin=%u chan=%p retChan=%d retEnc=%d retEnable=%d\n",
                          _pin, (void*)_channel, (int)retChan, (int)retEnc, (int)retEnable);
        } else {
            Serial.printf("DBG RMT Initialize ok pin=%u chan=%p\n", _pin, (void*)_channel);
        }
    }
