...
- `release/*.full.bin` — **merged full flash image** generated by `esptool.py merge_bin`:
  combines `bootloader.bin` (at 0x0 or 0x1000 depending on chip) + `partitions.bin` (0x8000) +
  `firmware.bin` (0x10000) into a single image suitable for first-time UART flashing.
